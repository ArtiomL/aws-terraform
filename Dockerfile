...
RUN wget -O terraform.zip $(curl -sL https://releases.hashicorp.com/index.json | jq '{terraform}' | egrep "linux.*amd64" | grep -v "alpha\|beta\|-rc" | sort --version-sort -r | head -1 | awk -F[\"] '{print $4}') && \
	unzip terraform.zip && \
	rm terraform.zip && \
